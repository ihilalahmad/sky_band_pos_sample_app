...
EXPORT int pack(char *inputReqData, int transactionType, char *szSignature, char *szEcrBuffer) {
    ...
    if ((transactionType != TYPE_RECONCILATION) && (transactionType != TYPE_REGISTER) &&
        (transactionType != TYPE_REFUND)
        && (transactionType != TYPE_PRECOMP) && (transactionType != TYPE_PREAUTH_EXT) &&
        (transactionType != TYPE_PREAUTH_VOID)
        && (transactionType != TYPE_PARAM_DOWNLOAD) && (transactionType != TYPE_GET_PARAM) &&
        (transactionType != TYPE_SET_TERM_LANG)
        && (transactionType != TYPE_SET_PARAM) && (transactionType != TYPE_REVERSAL) &&
        (transactionType != TYPE_START_SESSION)
        && (transactionType != TYPE_END_SESSION) && (transactionType != TYPE_PRNT_DETAIL_RPORT) &&
        (transactionType != TYPE_PRNT_SUMMARY_RPORT)
        && (transactionType != TYPE_REPEAT) && (transactionType != TYPE_CHECK_STATUS) &&
        (transactionType != TYPE_PARTIAL_DOWNLOAD)
        ...
        if (transactionType == TYPE_BILL_PAY) {
            ...
            memcpy(&szEcrBuffer[inReqPacketIndex], FIELD_SEPERATOR,
                   FIELDSEP_SIZE); // Field separator
            inReqPacketIndex++;
        ...
        if (transactionType == TYPE_PURCHASE_CASHBACK) {
            ...
            memcpy(&szEcrBuffer[inReqPacketIndex], szCashbackAmountField,
                   AMT_SIZE); // Cash Back Amount
            inReqPacketIndex += AMT_SIZE;
            memcpy(&szEcrBuffer[inReqPacketIndex], FIELD_SEPERATOR,
                   FIELDSEP_SIZE); // Field separator
            inReqPacketIndex++;
    ...
    if ((transactionType == TYPE_REGISTER) || (transactionType == TYPE_START_SESSION) ||
        (transactionType == TYPE_END_SESSION)) {
    ...
    if ((transactionType == TYPE_REFUND) || (transactionType == TYPE_PRECOMP)
        || (transactionType == TYPE_PREAUTH_EXT) || (transactionType == TYPE_PREAUTH_VOID) /*||
        (transactionType == TYPE_REVERSAL)*/) {
    ...
    if ((transactionType == TYPE_PRECOMP) || (transactionType == TYPE_PREAUTH_EXT) ||
        (transactionType == TYPE_PREAUTH_VOID)) {
        //Original Tran Date
    ...
    if ((transactionType != TYPE_REGISTER) && (transactionType != TYPE_START_SESSION) &&
        ...
        if (transactionType == TYPE_PURCHASE || transactionType == TYPE_PREAUTH ||
            transactionType == TYPE_CASH_ADVANCE /*|| transactionType == TYPE_REVERSAL*/)
            ...
            sprintf(szECRRefNum, "%s", szReqFields[4]);
                 ...
                 transactionType == TYPE_REPEAT
                 || (transactionType == TYPE_PRNT_SUMMARY_RPORT) || transactionType == TYPE_REVERSAL)
            sprintf(szECRRefNum, "%s", szReqFields[2]);
        else if ((transactionType == TYPE_REFUND) || (transactionType == TYPE_PREAUTH_EXT) ||
                 (transactionType == TYPE_BILL_PAY))
        ...
        else if ((transactionType == TYPE_PARAM_DOWNLOAD) || (transactionType == TYPE_GET_PARAM) ||
                 (transactionType == TYPE_PRNT_DETAIL_RPORT)
        ...
        if ((transactionType != TYPE_PARAM_DOWNLOAD) && (transactionType != TYPE_GET_PARAM) &&
            (transactionType != TYPE_SET_TERM_LANG)
            && (transactionType != TYPE_SET_PARAM) && (transactionType != TYPE_PRNT_DETAIL_RPORT) &&
            (transactionType != TYPE_PRNT_SUMMARY_RPORT)
            && (transactionType != TYPE_REPEAT) && (transactionType != TYPE_CHECK_STATUS) &&
            (transactionType != TYPE_PARTIAL_DOWNLOAD)
            && (transactionType != TYPE_SNAPSHOT_TOTAL)) {
